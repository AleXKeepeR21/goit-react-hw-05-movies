...
export function fetchGetTrending(page = 1) {
  return fetch(`${apiUrl}/trending/movie/day?api_key=${apiKey}&page=${page}`);
}

export function fetchSearchMovies(query, page = 1) {
  return fetch(`${apiUrl}/search/movie?api_key=${apiKey}&query=${query}&language=en-US&page=${page}&include_adult=false
    `);
}

export function fetchGetMovieDetails(id) {
  return fetch(`${apiUrl}/movie/${id}?api_key=${apiKey}&language=en-US
`);
}

export function FetchGetMovieCredits(id) {
  return fetch(`${apiUrl}/movie/${id}/credits?api_key=${apiKey}&language=en-US
  `);
}

export function FetchGetMovieReviews(id, page = 1) {
  return fetch(`${apiUrl}/movie/${id}/reviews?api_key=${apiKey}&language=en-US&page=${page}
  `);
}
